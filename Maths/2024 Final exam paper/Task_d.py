...
def sysfuncs(X):
    ...
    UV[0] = (X[0])**2 +(X[1])**2 + X[2]**2 - 1.85**2
    UV[1] = (X[0]-1.25)**2 +(X[1])**2 + X[2]**2 - 1.5**2
    UV[2] = (X[0]-0.8)**2 +(X[1]-0.8)**2 + X[2]**2 -1.85**2
    return UV
...
# set the accuracy requested, in this case it needs to be accruate to 2 dp
eps = 0.005
# set the initial guess
X0 = np.array([0.2, 1,0.4])

# set dx (of the order of or smaller than eps)
Dx = np.array([0.1*eps, 0.1*eps,0.1*eps])
